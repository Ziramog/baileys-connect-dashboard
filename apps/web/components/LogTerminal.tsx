    ...
    <div className="bg-black rounded-lg border border-zinc-800 font-mono text-xs overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-800 bg-zinc-900">
        ...
        <button
          ...
          className={`px-2 py-1 rounded text-xs ${paused ? 'bg-yellow-500/20 text-yellow-400' : 'bg-zinc-800 text-zinc-400'}`}
        >
          {paused ? 'Resume'}
        </button>
      </div>
      ...
      >
        ...
        {logs.map((line, i) => {
          const isError = /\berror\b|\bfail\b|\bwARN\b/i.test(line)
          const isOk = /\bconnected\b|\bstarted\b|\bok\b/i.test(line)
          return (
            <div
              key={i}
              className={`leading-relaxed ${
                isError ? 'text-red-400' : isOk ? 'text-green-400' : 'text-zinc-300'
              }`}
            >
              {line}
            </div>
